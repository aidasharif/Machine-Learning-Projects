...
def preprocess_spambase(spambase)
    bspambase = spambase.clone
    bspambase["data"] = spambase["data"].collect do |row|
      f = Hash.new
      f["bias"] = 1.0
      r = row.clone
      row["features"].keys
        .select {|k| k.start_with?('word') or k.start_with?('char')}
        .each {|k| f[k] = row["features"][k] = 1.0}
      r["features"] = f
      r
    end
    bspambase
